...
Response format:
{
  "metric": "correctness",
  "evaluations": [
    {"req_id": "R1", "score": 4, "reason": "Accurately reflects login flow described in section 2."},
    ...
  ]
}""",
...
Response format:
{
  "metric": "completeness",
  "evaluations": [
    {"req_id": "R1", "score": 3, "reason": "Missing error handling when email already exists."},
    ...
  ]
}""",
...
Response format:
{
  "metric": "edge_cases",
  "evaluations": [
    {"req_id": "R1", "score": 2, "reason": "No mention of invalid email format or rate limiting."},
    ...
  ]
}""",
...
- If overlapping, name the other requirement ID in your reason.
- Keep each reason under 20 words.
- Return ONLY valid JSON. No markdown, no preamble, no explanation outside the JSON.
...
Response format:
{
  "metric": "redundancy",
  "evaluations": [
    {"req_id": "R1", "score": 2, "reason": "Overlaps significantly with R3 on email validation logic."},
    ...
  ]
}""",
...
Response format:
{
  "metric": "testability",
  "evaluations": [
    {"req_id": "R1", "score": 5, "reason": "Clear actor, action, and verifiable success condition defined."},
    ...
  ]
}""",
}

METRICS = list(METRIC_PROMPTS.keys())
